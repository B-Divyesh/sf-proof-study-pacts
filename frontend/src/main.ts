...
function renderWorkspace(pact: Pact, token: string, demo: boolean): void {
  ...
  const sampleAttempt = pact.attempts[0];
  app.innerHTML = shell(`<main id="main" class="workspace${demo ? ' demo-workspace' : ''}">
    <section class="work-head"><div><p class="eyebrow"><span class="lamp active" aria-hidden="true"></span>${demo ? 'Sample session' : `Week of ${esc(formatDate(pact.weekOf))}`}</p><h1 tabindex="-1">${esc(pact.exerciseTitle)}</h1>${!demo ? `<p class="theorem"><code>${esc(pact.theorem)}</code></p><a href="${esc(pact.exerciseUrl)}" target="_blank" rel="noreferrer">Open the public exercise <span class="visually-hidden">(new tab)</span> ↗</a>` : ''}</div><div class="session-dial" aria-label="Session status: ${esc(pact.status)}"><span class="needle ${pact.status}"></span><strong>${esc(pact.status)}</strong></div></section>
    ${demo && sampleAttempt ? `<section class="demo-first-look" aria-labelledby="demo-first-look-title">
      <ul class="demo-role-list" aria-label="Sample partner roles">${pact.members.map(member => `<li><strong>${esc(member.name)} — ${esc(member.role)}</strong><span>${member.joined ? 'Ready' : 'Waiting'}</span></li>`).join('')}</ul>
      <article class="demo-attempt-preview"><div><p class="eyebrow">Saved sample attempt</p><h2 id="demo-first-look-title">${esc(sampleAttempt.author)}’s saved attempt</h2></div><code>${esc(sampleAttempt.proofText.trim().split('\n')[0])} …</code><p>${esc(sampleAttempt.explanation)}</p><a href="#attempt-${sampleAttempt.id}">Read both sample attempts</a></article>
    </section>
    <section class="exercise-details" aria-labelledby="exercise-details-title"><p class="eyebrow">Public exercise</p><h2 id="exercise-details-title">Exercise details</h2><p class="theorem"><code>${esc(pact.theorem)}</code></p><a href="${esc(pact.exerciseUrl)}" target="_blank" rel="noreferrer">Open the public exercise <span class="visually-hidden">(new tab)</span> ↗</a></section>` : ''}
